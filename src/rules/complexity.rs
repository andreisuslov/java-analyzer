...
impl Rule for S1200ClassCoupling {
    ...
    fn title(&self) -> &str {
        "Classes should not be coupled to too many classes"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static IMPORT_RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"import\s+[\w.]+\.(\w+)\s*;").unwrap());
                ...
                if let Some(start) = case_start {
                    ...
                    if case_lines > MAX_CASE_LINES {
                        issues.push(create_issue(
                            ...
                            1,
                            format!(
                                "Case has {} lines (max: {}). Extract to a method.",
                                case_lines, MAX_CASE_LINES
                            ),
                            None,
                        ));
                    }
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            ...
            if total > MAX_OPERATORS {
                issues.push(create_issue(
                    ...
                    1,
                    format!(
                        "Expression has {} logical operators (max: {}). Extract to variables.",
                        total, MAX_OPERATORS
                    ),
...
complexity_rule!(
    S1070ManyCtors,
    "S1070",
    "Class has too many constructors",
    Severity::Major,
    r"public\s+\w+\s*\([^)]*\)\s*\{[^}]+public\s+\w+\s*\(",
...
complexity_rule!(
    ...
    Severity::Major,
    r"\(\s*\w+\s*,\s*\w+\s*,\s*\w+\s*,\s*\w+\s*,\s*\w+\s*\)\s*->",
    "Lambda has too many parameters."
);
...
complexity_rule!(
    ...
    Severity::Major,
    r"(?:int|String|boolean|double|long)\s+\w+\s*[=;].*(?:int|String|boolean)\s+\w+\s*[=;].*(?:int|String|boolean)",
...
complexity_rule!(
    S1076DeepStream,
    "S1076",
    "Deeply nested stream operations",
    Severity::Major,
    r"\.stream\(\)[^;]*\.flatMap[^;]*\.flatMap",
...
complexity_rule!(
    ...
    Severity::Major,
    r"catch\s*\([^}]+catch\s*\([^}]+catch\s*\([^}]+catch\s*\(",
    "Consider multi-catch or restructuring."
);

// S1078: Complex boolean assignment
complexity_rule!(
    S1078ComplexBoolAssign,
    "S1078",
    "Complex boolean assignment",
    Severity::Major,
    r"boolean\s+\w+\s*=\s*[^;]*(?:&&|\|\|)[^;]*(?:&&|\|\|)[^;]*(?:&&|\|\|)",
...
complexity_rule!(
    ...
    "Deep if nesting",
    Severity::Major,
    r"if\s*\([^{]+\{[^}]*if\s*\([^{]+\{[^}]*if\s*\([^{]+\{[^}]*if\s*\(",
    "Reduce if nesting depth."
);

// S1082: Complex annotation usage
complexity_rule!(
    S1082ComplexAnnotation,
    "S1082",
    "Complex annotation usage",
    Severity::Minor,
    r"@\w+\s*\([^)]{100,}\)",
...
complexity_rule!(
    ...
    Severity::Major,
    r"return\s+[^;]+;[^}]+return\s+[^;]+;[^}]+return\s+[^;]+;[^}]+return\s+[^;]+;[^}]+return",
...
complexity_rule!(
    S1084ManyBreaks,
    "S1084",
    "Too many break or continue statements",
    Severity::Major,
    r"(?:break|continue)\s*;[^}]*(?:break|continue)\s*;[^}]*(?:break|continue)\s*;",
...
complexity_rule!(
    ...
    Severity::Minor,
    r"import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;\s*import\s+[\w.]+\s*;",
...
complexity_rule!(
    S1087ComplexEnum,
    "S1087",
    "Complex enum with too many members",
    Severity::Major,
    r"enum\s+\w+\s*\{[^}]{500,}\}",
...
complexity_rule!(
    S1088ComplexInterface,
    "S1088",
    "Interface is too complex",
    Severity::Major,
    r"interface\s+\w+[^{]*\{[^}]{1000,}\}",
    "Split large interface."
);

// S1089: Too many annotations
complexity_rule!(
    S1089ManyAnnotations,
    "S1089",
    "Too many annotations on element",
    Severity::Minor,
    r"@\w+[^@]*@\w+[^@]*@\w+[^@]*@\w+[^@]*@\w+",
...
complexity_rule!(
    S1090ComplexException,
    "S1090",
    "Complex exception handling",
    Severity::Major,
    r"throws\s+\w+\s*,\s*\w+\s*,\s*\w+\s*,\s*\w+\s*,\s*\w+",
...
complexity_rule!(
    S1093ManyStaticImport,
    "S1093",
    "Too many static imports",
    Severity::Minor,
    r"import\s+static[^;]+;\s*import\s+static[^;]+;\s*import\s+static[^;]+;\s*import\s+static[^;]+;\s*import\s+static",
...
complexity_rule!(
    ...
    Severity::Major,
    r"class\s+\w+[^}]*class\s+\w+[^}]*class\s+\w+[^}]*class\s+\w+",
...
complexity_rule!(
    ...
    Severity::Minor,
    r"Map\.of\s*\([^)]{100,}\)|Map\.ofEntries\s*\([^)]{100,}\)",
...
complexity_rule!(
    S1098ManyTypeCast,
    "S1098",
    "Too many type casts in expression",
    Severity::Major,
    r"\(\s*\w+\s*\)[^;]*\(\s*\w+\s*\)[^;]*\(\s*\w+\s*\)",
...
complexity_rule!(
    ...
    "Deep class nesting",
    Severity::Major,
    r"class\s+\w+[^}]*\{[^}]*class\s+\w+[^}]*\{[^}]*class\s+\w+",
...
complexity_rule!(
    S1102ManyMethodCalls,
    "S1102",
    "Too many method calls in expression",
    Severity::Minor,
    r"\.\w+\(\)[^;]*\.\w+\(\)[^;]*\.\w+\(\)[^;]*\.\w+\(\)[^;]*\.\w+\(\)",
...
complexity_rule!(
    ...
    Severity::Major,
    r"\.stream\(\)[^;]*\.filter[^;]*\.map[^;]*\.filter[^;]*\.map",
...
complexity_rule!(
    ...
    Severity::Major,
    r"void\s+\w+\s*\([^)]*\)\s*\{[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)[^}]*(?:int|String|boolean)",
...
complexity_rule!(
    ...
    Severity::Minor,
    r"new\s+\w+\s*\([^)]*new\s+\w+\s*\([^)]*new\s+\w+",
    "Simplify nested object creation."
);

// S1113: Too many parameters in generic
complexity_rule!(
    S1113ManyGenericParams,
    "S1113",
    "Too many parameters in generic",
    Severity::Minor,
...
complexity_rule!(
    ...
    Severity::Minor,
    r"\.substring[^;]+\.substring[^;]+\.substring",
    "Simplify string operations."
);

// S1116B: Complex collection operation
complexity_rule!(
    S1116BComplexCollection,
    "S1116B",
    "Complex collection operation",
    Severity::Major,
    r"\.addAll[^;]+\.removeAll[^;]+\.retainAll",
...
mod tests {
    ...
    fn test_s3776_simple_method() {
        ...
        assert!(
            issues.is_empty(),
            "Simple method should not trigger complexity warning"
        );
    }
