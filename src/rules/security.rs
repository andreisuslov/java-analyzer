...
impl Rule for S2076CommandInjection {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static PROCESS_BUILDER: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"ProcessBuilder\s*\(").unwrap());
        let mut issues = Vec::new();
            ...
            if RUNTIME_EXEC.is_match(line) || PROCESS_BUILDER.is_match(line) {
                ...
                if line.contains("+") || line.contains("concat") {
                    issues.push(create_issue(
                        ...
                        line_num + 1,
                        1,
                        "Potential command injection vulnerability. Validate and sanitize input."
                            .to_string(),
                        Some(line.trim().to_string()),
                    ));
                }
...
impl Rule for S2078LdapInjection {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static LDAP_SEARCH: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.(search|lookup)\s*\([^)]*\+").unwrap());
        let mut issues = Vec::new();

        for (line_num, line) in ctx.source.lines().enumerate() {
            if (line.contains("DirContext") || line.contains("LdapContext"))
                && LDAP_SEARCH.is_match(line)
            {
                issues.push(create_issue(
                    self,
                    ctx.file_path,
                    line_num + 1,
                    1,
                    "Potential LDAP injection vulnerability. Use parameterized queries."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S2083PathInjection {
    ...
    fn title(&self) -> &str {
        "I/O function calls should not be vulnerable to path injection"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if FILE_OPS.is_match(line) {
                issues.push(create_issue(
                    ...
                    line_num + 1,
                    1,
                    "Potential path injection vulnerability. Validate and canonicalize paths."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S2245PseudoRandomGenerator {
    ...
    fn title(&self) -> &str {
        "Secure random generators should be used for security"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static WEAK_RANDOM: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"new\s+Random\s*\(|Math\s*\.\s*random\s*\(").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S2257CustomCrypto {
    ...
    fn title(&self) -> &str {
        "Custom cryptographic algorithms should not be used"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
...
impl Rule for S2386MutableStaticField {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static MUTABLE_STATIC: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"public\s+static\s+(\w+(\[\]|<[^>]+>)?)\s+\w+").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S3329IvReuse {
    ...
    fn title(&self) -> &str {
        "Cipher IVs should be unpredictable"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static IV_PATTERN: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"IvParameterSpec\s*\(\s*new\s+byte\s*\[").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S3649SqlInjection {
    ...
    fn title(&self) -> &str {
        "Database queries should not be vulnerable to injection"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static SQL_CONCAT: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r#"(executeQuery|executeUpdate|execute|prepareStatement)\s*\([^)]*\+"#)
                .unwrap()
        });
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if SQL_CONCAT.is_match(line) || SQL_LITERAL.is_match(&line.to_uppercase()) {
                issues.push(create_issue(
                    ...
                    line_num + 1,
                    1,
                    "SQL injection vulnerability. Use PreparedStatement with parameters."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S4426CryptoKeySize {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static WEAK_KEY: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.init\s*\(\s*(512|768|1024)\s*\)").unwrap());
        let mut issues = Vec::new();

        for (line_num, line) in ctx.source.lines().enumerate() {
            if WEAK_KEY.is_match(line) && line.contains("KeyGenerator")
                || line.contains("KeyPairGenerator")
            {
                issues.push(create_issue(
                    self,
                    ctx.file_path,
...
impl Rule for S4502DisabledCsrf {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static CSRF_DISABLE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.csrf\s*\(\s*\)\s*\.\s*disable\s*\(").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S4790WeakHashing {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static WEAK_HASH: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r#"MessageDigest\s*\.\s*getInstance\s*\(\s*"(MD5|SHA-?1|SHA1)"\s*\)"#)
                .unwrap()
        });
        let mut issues = Vec::new();
...
impl Rule for S5144OpenRedirect {
    ...
    fn title(&self) -> &str {
        "Open redirects should be prevented"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S5146HttpResponseSplitting {
    ...
    fn title(&self) -> &str {
        "HTTP response headers should not be vulnerable to injection"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static HEADER_SET: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.(setHeader|addHeader)\s*\([^,]+,\s*[^)]*\+").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S5324ExternalStorage {
    ...
    fn title(&self) -> &str {
        "Using external storage is security-sensitive"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static EXT_STORAGE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"getExternalFilesDir|getExternalStorageDirectory").unwrap());
        let mut issues = Vec::new();
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if EXT_STORAGE.is_match(line) {
                issues.push(create_issue(
                    ...
                    line_num + 1,
                    1,
                    "External storage is world-readable. Don't store sensitive data there."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S5542EncryptionAlgorithm {
    ...
    fn title(&self) -> &str {
        "Encryption should use secure mode and padding"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static WEAK_CIPHER: Lazy<Regex> = Lazy::new(|| {
            Regex::new(
                r#"Cipher\s*\.\s*getInstance\s*\(\s*"(DES|DESede|RC2|RC4|Blowfish|AES)"\s*\)"#,
            )
            .unwrap()
        });
        let mut issues = Vec::new();
...
impl Rule for S5547CipherBlockMode {
    ...
    fn title(&self) -> &str {
        "Cipher block mode should be robust"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S5689HttpResponseHeaders {
    ...
    fn title(&self) -> &str {
        "Sensitive info should not be disclosed in HTTP headers"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static SERVER_HEADER: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#"setHeader\s*\(\s*"Server""#).unwrap());
        let mut issues = Vec::new();
...
impl Rule for S5808AuthorizationCheck {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static PERMIT_ALL: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.permitAll\s*\(\s*\)").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S6096ZipSlip {
    ...
    fn title(&self) -> &str {
        "Archive extraction should be secure (Zip Slip)"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static ZIP_ENTRY: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"ZipEntry|ZipInputStream|TarArchiveEntry").unwrap());
        let mut issues = Vec::new();

        if ZIP_ENTRY.is_match(ctx.source)
            && !ctx.source.contains("normalize")
            && !ctx.source.contains("getCanonicalPath")
        {
            for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S2091XPathInjection {
    ...
    fn title(&self) -> &str {
        "XPath expressions should not be vulnerable to injection"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
...
impl Rule for S2115DatabasePassword {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static EMPTY_PASS: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#"getConnection\s*\([^)]*,\s*""\s*\)"#).unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if EMPTY_PASS.is_match(line)
                || (line.contains("getConnection") && line.contains("null"))
            {
                issues.push(create_issue(
                    self,
                    ctx.file_path,
...
impl Rule for S2277RsaPadding {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static WEAK_RSA: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r#"Cipher\.getInstance\s*\(\s*"RSA(?:/ECB)?(?:/PKCS1Padding)?"\s*\)"#)
                .unwrap()
        });
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if WEAK_RSA.is_match(line) {
                issues.push(create_issue(
                    ...
                    line_num + 1,
                    1,
                    "Use RSA with OAEP padding (RSA/ECB/OAEPWithSHA-256AndMGF1Padding)."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S2278WeakDes {
    ...
    fn title(&self) -> &str {
        "DES and 3DES should not be used"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
...
impl Rule for S2631RegexDos {
    ...
    fn title(&self) -> &str {
        "Regex should not be vulnerable to ReDoS"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S2647BasicAuth {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if line.contains("httpBasic()")
                || line.contains("BasicAuth")
                || line.contains("Authorization: Basic")
            {
                issues.push(create_issue(
                    self,
                    ctx.file_path,
...
impl Rule for S2976TempFileCreation {
    ...
    fn title(&self) -> &str {
        "Use Files.createTempDirectory for temp directories"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S3330CookieDomain {
    ...
    fn title(&self) -> &str {
        "Cookie domain should be properly set"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static COOKIE_DOMAIN: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#"\.setDomain\s*\(\s*"\."#).unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S4434UncheckedDeserialize {
    ...
    fn title(&self) -> &str {
        "Deserialization should be done safely"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static DESERIALIZE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"ObjectInputStream|XMLDecoder|readObject\s*\(").unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S4507DebugFeatures {
    ...
    fn title(&self) -> &str {
        "Debug features should be deactivated"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S4684SpringEntity {
    ...
    fn title(&self) -> &str {
        "Persistent entities should not be used directly"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S4818SocketUsage {
    ...
    fn title(&self) -> &str {
        "Socket usage is security-sensitive"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S4823CommandLine {
    ...
    fn title(&self) -> &str {
        "Command line args should be validated"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S4829StdinUsage {
    ...
    fn title(&self) -> &str {
        "Standard input should be validated"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S4830TrustManager {
    ...
    fn title(&self) -> &str {
        "TrustManager should verify certificates"
    }
    fn severity(&self) -> Severity {
        Severity::Blocker
    }
...
impl Rule for S5042ExpansionInjection {
    ...
    fn title(&self) -> &str {
        "Archive expansion limits should be set"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        if ctx.source.contains("ZipInputStream") || ctx.source.contains("GZIPInputStream") {
            let has_limit = ctx.source.contains("limit")
                || ctx.source.contains("MAX_")
                || ctx.source.contains("threshold");
            if !has_limit {
                for (line_num, line) in ctx.source.lines().enumerate() {
                    if line.contains("ZipInputStream") || line.contains("GZIPInputStream") {
                        issues.push(create_issue(
                            ...
                            line_num + 1,
                            1,
                            "Set size limits when extracting archives (zip bomb protection)."
                                .to_string(),
                            Some(line.trim().to_string()),
                        ));
                    }
...
impl Rule for S5145LogInjection {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static LOG_CONCAT: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"(log|logger|LOG|LOGGER)\.(info|debug|warn|error|trace)\s*\([^)]*\+")
                .unwrap()
        });
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S5167HttpRequestForward {
    ...
    fn title(&self) -> &str {
        "HTTP request forwarding should be safe"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S5247SqlFormatter {
    ...
    fn title(&self) -> &str {
        "SQL formatters should be avoided"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S5261RegexCompile {
    ...
    fn title(&self) -> &str {
        "Regex patterns should be compiled once"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S5332ClearTextProtocol {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static CLEAR_TEXT: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#""(http|ftp|telnet|smtp)://[^"]+""#).unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if CLEAR_TEXT.is_match(line)
...
impl Rule for S5344PasswordHash {
    ...
    fn title(&self) -> &str {
        "Password hashing should use proper settings"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if line.contains("BCrypt") || line.contains("PBKDF2") || line.contains("Argon2") {
                if line.contains("cost") || line.contains("iterations") {
                    let cost_match = Regex::new(r"cost\s*[=:]\s*(\d+)")
                        .ok()
...
impl Rule for S5361RegexReplace {
    ...
    fn title(&self) -> &str {
        "replaceAll should use literal replacement"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
...
impl Rule for S5445InsecureTempFile {
    ...
    fn title(&self) -> &str {
        "Temp files should be created securely"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S5852RegexComplexity {
    ...
    fn title(&self) -> &str {
        "Regex complexity should be limited"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static COMPLEX_REGEX: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#"Pattern\.compile\s*\(\s*"[^"]{100,}""#).unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S5869CharacterClassRedundancy {
    ...
    fn title(&self) -> &str {
        "Character classes should not have redundancy"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
...
impl Rule for S6287RegexReplace2 {
    ...
    fn title(&self) -> &str {
        "Regex should not cause stack overflow"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S6293RegexComplex {
    ...
    fn title(&self) -> &str {
        "Regex should not be overly complex"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
