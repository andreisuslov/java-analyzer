...
use crate::rules::{
    create_issue, AnalysisContext, Issue, Lazy, Regex, Rule, RuleCategory, Severity,
};
...
smell_rule!(
    S1103HtmlEntities,
    "S1103",
    "HTML entities should not be used in comments",
    Severity::Minor,
    r"//.*&(?:amp|lt|gt|quot);",
...
smell_rule!(
    ...
    Severity::Major,
    r"^\s*\w+\s*:\s*(?:for|while|do|switch)",
    "Refactor to avoid using labels."
);

// S1121: Assignments should not be made from within sub-expressions
smell_rule!(
    S1121AssignmentInSubExpression,
    "S1121",
    "Assignments should not be in sub-expressions",
    Severity::Major,
    r"(?:if|while)\s*\([^)]*\w+\s*=\s*\w+[^=]",
    "Extract assignment from this sub-expression."
);

// S1123: Deprecated should have both annotation and Javadoc
smell_rule!(
    S1123DeprecatedMissing,
    "S1123",
    "@Deprecated should have both annotation and Javadoc",
    Severity::Major,
    r"@Deprecated\s*\n",
...
smell_rule!(
    S1124ModifierOrder,
    "S1124",
    "Modifiers should be in correct order",
    Severity::Minor,
    r"(?:final|static)\s+(?:public|private|protected)",
    "Reorder modifiers: public/protected/private should come first."
);

// S1125: Boolean literals should not be redundant
smell_rule!(
    S1125RedundantBoolean,
    "S1125",
    "Boolean literals should not be redundant",
    Severity::Minor,
    r"==\s*true|!=\s*false|true\s*==|false\s*!=",
...
smell_rule!(
    S1126BooleanReturn,
    "S1126",
    "Boolean return should not be wrapped",
    Severity::Minor,
    r"if\s*\([^)]+\)\s*(?:return\s+true|return\s+false)",
...
smell_rule!(
    S1147SystemExit,
    "S1147",
    "System.exit should not be called",
    Severity::Major,
    r"System\.exit\s*\(",
...
smell_rule!(
    S1149SynchronizedCollection,
    "S1149",
    "Synchronized classes should not be used",
    Severity::Major,
    r"new\s+(?:Vector|Hashtable|Stack|StringBuffer)\s*[<(]",
    "Use modern alternatives like ArrayList, HashMap, Deque, StringBuilder."
);

// S1150: Enumeration should not be implemented
smell_rule!(
    S1150EnumerationInterface,
    "S1150",
    "Enumeration should not be implemented",
    Severity::Major,
    r"implements\s+(?:\w+\s*,\s*)*Enumeration",
...
smell_rule!(
    S1153StringValueOf,
    "S1153",
    "String.valueOf() appended to String is redundant",
    Severity::Minor,
    r#"\+\s*String\.valueOf\s*\("#,
...
smell_rule!(
    S1157CaseInsensitiveCompare,
    "S1157",
    "Use equalsIgnoreCase for case-insensitive comparison",
    Severity::Major,
    r"\.toLowerCase\(\)\.equals\(|\.toUpperCase\(\)\.equals\(",
    "Use equalsIgnoreCase() instead of converting case."
);

// S1158: Primitive wrappers should not be instantiated
smell_rule!(
    S1158PrimitiveWrapper,
    "S1158",
    "Primitive wrappers should not be instantiated",
    Severity::Major,
    r"new\s+(?:Integer|Long|Double|Float|Boolean|Byte|Short|Character)\s*\(",
    "Use valueOf() or autoboxing instead of new wrapper()."
);

// S1160: Public methods should throw at most one checked exception
smell_rule!(
    S1160MultipleCheckedExceptions,
    "S1160",
    "Throw at most one checked exception",
    Severity::Major,
    r"throws\s+\w+(?:Exception)?\s*,\s*\w+(?:Exception)?",
    "Reduce the number of checked exceptions thrown."
);

// S1161: @Override should be used
smell_rule!(
    S1161MissingOverride,
    "S1161",
    "@Override should be used on overriding methods",
    Severity::Major,
    r"public\s+(?:boolean\s+equals|int\s+hashCode|String\s+toString)\s*\(",
    "Add @Override annotation to this method."
);

// S1162: Checked exceptions should not be thrown
smell_rule!(
    S1162CheckedException,
    "S1162",
    "Avoid throwing checked exceptions",
    Severity::Major,
    r"throw\s+new\s+(?:IOException|SQLException|ClassNotFoundException|InterruptedException)\s*\(",
...
smell_rule!(
    S1163ThrowInFinally,
    "S1163",
    "Exceptions should not be thrown in finally",
    Severity::Blocker,
    r"finally\s*\{[^}]*throw\s+",
...
impl Rule for S1164CatchRethrow {
    ...
    fn title(&self) -> &str {
        "Exceptions should not be caught and rethrown"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
...
impl Rule for S1165ExceptionNonFinalField {
    ...
    fn title(&self) -> &str {
        "Exception class fields should be final"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static FIELD_RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(?:private|protected|public)\s+(\w+)\s+\w+").unwrap());
        let mut issues = Vec::new();
...
smell_rule!(
    S1166ExceptionCauseNotPreserved,
    "S1166",
    "Preserve original exception as cause",
    Severity::Critical,
    r"catch\s*\([^)]+\)\s*\{[^}]*throw\s+new\s+\w+Exception\s*\([^,)]*\)",
...
impl Rule for S1170PublicConstant {
    ...
    fn title(&self) -> &str {
        "Move public constants to interface or utility class"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"public\s+static\s+final\s+(\w+)").unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if let Some(cap) = RE.captures(line) {
                ...
                if next_word != "class" && !line.trim().starts_with("//") {
                    issues.push(create_issue(
                        ...
                        line_num + 1,
                        1,
                        "Consider moving this constant to a dedicated constants interface."
                            .to_string(),
                        Some(line.trim().to_string()),
                    ));
                }
...
smell_rule!(
    S1171InstanceInitializer,
    "S1171",
    "Instance initializers should be avoided",
    Severity::Minor,
    r"^\s*\{[^}]+\}\s*$",
...
smell_rule!(
    S1174FinalizePublic,
    "S1174",
    "Object.finalize() should remain protected",
    Severity::Major,
    r"public\s+(?:void\s+)?finalize\s*\(\s*\)",
...
impl Rule for S1182CloneWithoutCloneable {
    ...
    fn title(&self) -> &str {
        "Classes overriding clone should implement Cloneable"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
smell_rule!(
    S1185UselessOverride,
    "S1185",
    "Don't override just to call super",
    Severity::Minor,
    r"@Override[^}]+\{\s*(?:return\s+)?super\.\w+\([^)]*\)\s*;\s*\}",
...
smell_rule!(
    S1189AssertAsVariable,
    "S1189",
    "assert should not be used as a variable name",
    Severity::Critical,
    r"(?:int|String|boolean|Object)\s+assert\s*[=;]",
...
smell_rule!(
    ...
    Severity::Major,
    r"(?:int|String|Object)\s+(?:enum|module|exports|opens|requires|uses|provides)\s*[=;]",
...
smell_rule!(
    S1191SunPackages,
    "S1191",
    "Classes from sun.* should not be used",
    Severity::Major,
    r"import\s+sun\.|sun\.\w+\.\w+",
...
smell_rule!(
    S1193ExceptionInstanceof,
    "S1193",
    "Don't test exception type with instanceof",
    Severity::Major,
    r"instanceof\s+\w*Exception",
...
smell_rule!(
    ...
    Severity::Major,
    r"extends\s+(?:Error|java\.lang\.Error)",
    "Extend Exception or RuntimeException instead of Error."
);

// S1195: Array designators should be on the type
smell_rule!(
    S1195ArrayDesignator,
    "S1195",
    "Array designators should be on the type",
    Severity::Minor,
    r"(?:int|String|Object)\s+\w+\[\]",
...
smell_rule!(
    S1214ConstantInterface,
    "S1214",
    "Don't use interfaces for constants only",
    Severity::Minor,
    r"interface\s+\w+\s*\{[^}]*(?:static\s+final|final\s+static)[^}]*\}",
...
smell_rule!(
    S1215SystemGc,
    "S1215",
    "System.gc() should not be called",
    Severity::Major,
    r"System\.gc\s*\(\s*\)|Runtime\.getRuntime\(\)\.gc\(\)",
...
impl Rule for S1219NonCaseLabel {
    ...
    fn title(&self) -> &str {
        "Switch should not contain non-case labels"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        if !has_package {
            issues.push(create_issue(
                ...
                1,
                1,
                "Add a package declaration - avoid the default package.".to_string(),
                None,
            ));
        }
        issues
...
impl Rule for S1223MethodNamedAsClass {
    ...
    fn title(&self) -> &str {
        "Methods should not have same name as class"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static METHOD_RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(?:public|private|protected)\s+(\w+)\s+(\w+)\s*\(").unwrap());
        let mut issues = Vec::new();
            ...
            if let Some(cap) = METHOD_RE.captures(line) {
                ...
                if method_name == class_name && return_type != "void" && !return_type.is_empty() {
                    issues.push(create_issue(
                        ...
                        line_num + 1,
                        1,
                        "This method has the same name as the class but is not a constructor."
                            .to_string(),
                        Some(line.trim().to_string()),
                    ));
                }
...
smell_rule!(
    S1301TooFewCases,
    "S1301",
    "Switch should have at least 3 cases",
    Severity::Minor,
    r"switch\s*\([^)]+\)\s*\{\s*case[^}]*\}",
...
smell_rule!(
    S1450FieldShouldBeLocal,
    "S1450",
    "Field used in one method should be local",
    Severity::Major,
    r"private\s+(?:static\s+)?(\w+)\s+temp\w*\s*;",
...
impl Rule for S1451MissingCopyright {
    ...
    fn title(&self) -> &str {
        "File should have a copyright header"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        if !first_lines.contains("Copyright") && !first_lines.contains("copyright") {
            issues.push(create_issue(
                ...
                1,
                1,
                "Add a copyright header to this file.".to_string(),
                None,
            ));
        }
        issues
...
smell_rule!(
    ...
    Severity::Major,
    r"(?:int|String|boolean|double|float|long)\s+unused\w*\s*=",
...
impl Rule for S1488ImmediateReturn {
    ...
    fn title(&self) -> &str {
        "Don't declare variable just to return it"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
                ...
                if let Some(ret_cap) = RETURN_RE.captures(lines[i + 1]) {
                    ...
                    if var_name == ret_var && !var_name.is_empty() {
                        issues.push(create_issue(
                            ...
                            i + 1,
                            1,
                            "Return the value directly instead of storing in a temporary variable."
                                .to_string(),
...
smell_rule!(
    S1643StringConcatInLoop,
    "S1643",
    "Use StringBuilder for String concat in loop",
    Severity::Major,
    r#"(?:for|while)[^{]+\{[^}]*\+\s*=\s*["']"#,
...
smell_rule!(
    S1659MultipleDeclarations,
    "S1659",
    "Declare one variable per line",
    Severity::Minor,
    r"(?:int|String|boolean|double|float|long|Object)\s+\w+\s*,\s*\w+",
...
impl Rule for S1694AbstractWithoutMethod {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        let mut issues = Vec::new();
        let has_abstract_method = ctx
            .source
            .lines()
            .any(|l| l.contains("abstract") && l.contains("(") && !l.contains("class"));
        for (line_num, line) in ctx.source.lines().enumerate() {
...
impl Rule for S1700FieldNamedAsClass {
    ...
    fn title(&self) -> &str {
        "Field/method name differs from class"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
smell_rule!(
    S2133UselessObject,
    "S2133",
    "Useless objects should not be created",
    Severity::Major,
    r"new\s+(?:Object|String)\s*\(\s*\)\s*;",
...
smell_rule!(
    S2154MixedWrappers,
    "S2154",
    "Don't mix dissimilar primitive wrappers",
    Severity::Critical,
    r"(?:Integer|Long|Double|Float)\.compare\w*\([^)]*(?:Integer|Long|Double|Float)",
...
impl Rule for S2160EqualsWithoutHashCode {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        let mut issues = Vec::new();
        let has_equals =
            ctx.source.contains("boolean equals(") || ctx.source.contains("boolean equals (");
        let has_hashcode =
            ctx.source.contains("int hashCode(") || ctx.source.contains("int hashCode (");
...
impl Rule for S2166ExceptionNaming {
    ...
    fn title(&self) -> &str {
        "Exception-named class should extend Exception"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S2176ClassShadowing {
    ...
    fn title(&self) -> &str {
        "Class names should not shadow parents"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"class\s+(\w+)\s+extends\s+\w+\s+implements\s+(.+)").unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
            if let Some(cap) = RE.captures(line) {
...
smell_rule!(
    S2232ResultSetIsLast,
    "S2232",
    "Don't use ResultSet.isLast()",
    Severity::Major,
    r"\.isLast\s*\(\s*\)",
...
smell_rule!(
    S2326UnusedTypeParameter,
    "S2326",
    "Remove unused type parameters",
    Severity::Major,
    r"<\s*[A-Z]\s*>\s*(?:class|interface)",
...
smell_rule!(
    S2388InnerShadowing,
    "S2388",
    "Inner class should not shadow outer",
    Severity::Major,
    r"class\s+\w+[^{]*\{[^}]*class\s+\w+[^{]*\{[^}]*\bthis\.",
...
smell_rule!(
    S2437SillyBitOp,
    "S2437",
    "Remove silly bit operations",
    Severity::Minor,
    r"(?:&\s*0\b|\|\s*0\b|\^\s*0\b|>>\s*0\b|<<\s*0\b)",
    "This bit operation has no effect - remove it."
);

// S2440: Classes with only static members should not be instantiated
smell_rule!(
    S2440StaticClassInstance,
    "S2440",
    "Don't instantiate static-only classes",
    Severity::Major,
    r"new\s+(?:Math|Collections|Arrays|System)\s*\(",
    "This class has only static members - don't instantiate it."
);

// S2479: Whitespace characters outside of string literals
smell_rule!(
    S2479SpecialWhitespace,
    "S2479",
    "Avoid special whitespace characters",
    Severity::Major,
...
smell_rule!(
    S3012VarargsArray,
    "S3012",
    "Don't create arrays for varargs",
    Severity::Minor,
    r#"\.format\s*\(\s*"[^"]+"\s*,\s*new\s+Object\s*\[\s*\]"#,
...
impl Rule for S3047MultipleLoops {
    ...
    fn title(&self) -> &str {
        "Combine multiple loops over same set"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static LOOP_RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"for\s*\([^)]+:\s*(\w+)\s*\)").unwrap());
        let mut issues = Vec::new();
            ...
            if let Some(cap) = LOOP_RE.captures(line) {
                ...
                if let Some(&first_line) = loop_vars.get(&collection) {
                    issues.push(create_issue(
                        ...
                        1,
                        format!(
                            "Consider combining this loop with the one at line {} over '{}'.",
                            first_line, collection
                        ),
                        Some(line.trim().to_string()),
                    ));
                } else {
...
smell_rule!(
    ...
    Severity::Minor,
    r"(?:private|public|protected)\s+(?:int|long|short|byte)\s+\w+\s*=\s*0\s*;",
...
impl Rule for S3077NonPrimitiveVolatile {
    ...
    fn title(&self) -> &str {
        "Non-primitives should not be volatile"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
            ...
            if let Some(cap) = RE.captures(line) {
                ...
                if !primitives.contains(&type_name) {
                    issues.push(create_issue(
                        ...
                        line_num + 1,
                        1,
                        "Volatile on non-primitives doesn't ensure thread-safety of contents."
                            .to_string(),
                        Some(line.trim().to_string()),
                    ));
                }
...
smell_rule!(
    S3398InlinePrivate,
    "S3398",
    "Consider inlining private method",
    Severity::Minor,
    r"private\s+(?:static\s+)?(?:final\s+)?void\s+helper\w*\s*\(",
...
smell_rule!(
    S3400ReturnConstant,
    "S3400",
    "Method returning constant should be a constant",
    Severity::Minor,
    r#"(?:return\s+\d+\s*;|return\s+"[^"]*"\s*;|return\s+(?:true|false)\s*;)"#,
...
smell_rule!(
    ...
    Severity::Major,
    r#"assertEquals\s*\(\s*\w+\s*,\s*(?:\d+|"[^"]*"|true|false)\s*\)"#,
...
smell_rule!(
    S3551ExceptionNotLogged,
    "S3551",
    "Log or rethrow exceptions",
    Severity::Major,
    r#"catch\s*\([^)]+\)\s*\{\s*//"#,
...
smell_rule!(
    S3553AssertThrows,
    "S3553",
    "Use specific assertThrows",
    Severity::Minor,
    r"assertThrows\s*\(\s*Exception\.class",
...
smell_rule!(
    ...
    Severity::Minor,
    r"continue\s*;\s*\}|break\s*;\s*default\s*:",
...
impl Rule for S3740RawType {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(List|Set|Map|Collection|Iterator)\s+\w+\s*[=;]").unwrap());
        let mut issues = Vec::new();
        for (line_num, line) in ctx.source.lines().enumerate() {
...
smell_rule!(
    ...
    Severity::Major,
    r"@(?:Autowired|Inject)\s+(?:private|protected)\s+\w+\s+\w+\s*;",
        ...
        for cap in RE.captures_iter(ctx.source) {
            if cap.get(1).map(|m| m.as_str()) == cap.get(2).map(|m| m.as_str()) {
                ...
                issues.push(create_issue(
                    ...
                    line_num,
                    1,
                    "Both branches of this if do the same thing.".to_string(),
                    None,
                ));
            }
        }
            ...
            for cap in RE.captures_iter(line) {
                ...
                if let Some(&first_line) = stream_vars.get(&var) {
                    issues.push(create_issue(
                        ...
                        1,
                        format!(
                            "Stream from '{}' was already consumed at line {}.",
                            var, first_line
                        ),
                        Some(line.trim().to_string()),
                    ));
...
smell_rule!(
    S3972ConditionalNewLine,
    "S3972",
    "Start conditionals on new lines",
    Severity::Minor,
    r";\s*if\s*\(",
...
smell_rule!(
    S3973ImplicitNullCheck,
    "S3973",
    "Null comparisons should be explicit",
    Severity::Minor,
    r"if\s*\(\s*\w+\s*\)\s*\{",
...
smell_rule!(
    S4143CollectionMethod,
    "S4143",
    "Check collection method usage",
    Severity::Critical,
    r"\.(?:add|remove|contains)\s*\(\s*\w+\s*,\s*\w+\s*\)",
...
smell_rule!(
    S4201NullInstanceof,
    "S4201",
    "Use instanceof for null-safe checks",
    Severity::Minor,
    r"!=\s*null\s*&&\s*\w+\s+instanceof",
    "The instanceof operator already handles null checks."
);

// S4449: Optional.isPresent() should be paired with get()
smell_rule!(
    S4449OptionalIsPresent,
    "S4449",
    "Use orElse instead of isPresent/get",
    Severity::Minor,
    r"\.isPresent\s*\(\s*\)[^}]*\.get\s*\(\s*\)",
...
smell_rule!(
    S4551TypeToken,
    "S4551",
    "TypeToken should be parameterized",
    Severity::Major,
    r"new\s+TypeToken\s*\(\s*\)",
...
smell_rule!(
    S4596PropertiesMap,
    "S4596",
    "Properties should be compatible with Maps",
    Severity::Major,
    r"Properties\s+\w+\s*=\s*new\s+HashMap",
...
smell_rule!(
    S4719MementoClass,
    "S4719",
    "Memento pattern classes in separate files",
    Severity::Minor,
    r"class\s+\w*Memento[^{]*\{",
...
smell_rule!(
    ...
    Severity::Major,
    r"catch\s*\(\s*NullPointerException",
...
smell_rule!(
    S5122CorsPolicy,
    "S5122",
    "CORS should be constrained",
    Severity::Major,
    r#"addAllowedOrigin\s*\(\s*"\*"\s*\)"#,
...
smell_rule!(
    ...
    Severity::Major,
    r#"\.matches\s*\(\s*"[^"]+"\s*\)|Pattern\.compile\s*\(\s*"[^"]+"\s*\)\s*\.\s*matcher"#,
...
smell_rule!(
    S5786JUnit5Public,
    "S5786",
    "JUnit5 tests don't need public modifier",
    Severity::Minor,
...
smell_rule!(
    S5838SimplifyAssert,
    "S5838",
    "Simplify assertEquals",
    Severity::Minor,
    r"assertEquals\s*\(\s*(?:true|false)\s*,",
...
smell_rule!(
    ...
    Severity::Minor,
    r"assert(?:True|False|Null|NotNull|Equals)\s*\([^,)]+\)",
...
smell_rule!(
    S5961LongTest,
    "S5961",
    "Test methods should be concise",
    Severity::Major,
    r"@Test[^}]{1000,}",
...
impl Rule for S5967DuplicateSignature {
    ...
    fn title(&self) -> &str {
        "Avoid duplicate method signatures"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
            ...
            if let Some(cap) = RE.captures(line) {
                ...
                if let Some(&first_line) = method_names.get(&method) {
                    issues.push(create_issue(
                        ...
                        1,
                        format!(
                            "Method '{}' appears to be duplicated (first at line {}).",
                            method, first_line
                        ),
                        Some(line.trim().to_string()),
                    ));
...
smell_rule!(
    S5969NegatedAssertion,
    "S5969",
    "Don't negate assertions",
    Severity::Minor,
    r"assertFalse\s*\(\s*!|assertTrue\s*\(\s*!",
...
smell_rule!(
    S5970ReturnThis,
    "S5970",
    "Avoid returning 'this' implicitly",
    Severity::Minor,
    r"return\s+this\s*;",
...
smell_rule!(
    S6201PatternFlags,
    "S6201",
    "Use Pattern constants for flags",
    Severity::Minor,
    r"Pattern\.compile\s*\([^)]+,\s*\d+\s*\)",
...
smell_rule!(
    S6204StreamEquality,
    "S6204",
    "Check stream comparison methods",
    Severity::Major,
    r"\.filter\s*\([^)]*==",
...
smell_rule!(
    S6212LocalTypeInference,
    "S6212",
    "Consider using var",
    Severity::Minor,
    r"(?:ArrayList|HashMap|HashSet)<[^>]+>\s+\w+\s*=\s*new\s+(?:ArrayList|HashMap|HashSet)",
    "Consider using 'var' for local variable type inference."
);

// S6213: Restricted types should not be used
smell_rule!(
    S6213RestrictedIdentifiers,
    "S6213",
    "Avoid restricted identifiers",
    Severity::Major,
    r"(?:int|String)\s+(?:_|var|yield|record|sealed|permits)\s*[=;]",
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if line.contains("@Test") {
                in_test = true;
                test_line = line_num + 1;
                has_assert = false;
            }
            if in_test && METHOD_RE.is_match(line) {
                brace_count = 0;
            }
            if in_test {
                ...
                if brace_count == 0 && line.contains('}') {
                    if !has_assert {
                        issues.push(create_issue(
                            ...
                            test_line,
                            1,
                            "Add assertions to verify test behavior.".to_string(),
                            None,
                        ));
                    }
                    in_test = false;
...
smell_rule!(
    S6353EmptyAlternation,
    "S6353",
    "Regex should not have empty alternation",
    Severity::Major,
...
smell_rule!(
    S6432SwitchArrows,
    "S6432",
    "Consider switch expressions with arrows",
    Severity::Minor,
    r"case\s+\w+\s*:\s*\n\s*return",
...
impl Rule for S106SystemOutUsed {
    ...
    fn title(&self) -> &str {
        "Standard outputs should not be used for logging"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"System\.(out|err)\.(print|println|printf|format)").unwrap());
...
impl Rule for S108EmptyBlock {
    ...
    fn title(&self) -> &str {
        "Nested blocks should not be empty"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"(?:if|else|for|while|try|catch|finally|switch)\s*(?:\([^)]*\))?\s*\{\s*\}")
                .unwrap()
        });
...
impl Rule for S109MagicNumber {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"[=<>+\-*/]\s*(\d{2,})[^dDfFlL]").unwrap());
...
impl Rule for S110DeepInheritance {
    ...
    fn title(&self) -> &str {
        "Inheritance tree should not be too deep"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"class\s+\w+\s+extends\s+\w+").unwrap());
        ...
        if extends_count > 5 {
            issues.push(create_issue(
                ...
                1,
                format!(
                    "This file has {} class extensions. Consider reducing inheritance depth.",
                    extends_count
                ),
                None,
            ));
...
impl Rule for S125CommentedOutCode {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static CODE_PATTERNS: Lazy<Vec<Regex>> = Lazy::new(|| {
            vec![
                Regex::new(r"//\s*(?:if|for|while|return|public|private|protected|class)\s*[\(\{]")
                    .unwrap(),
...
impl Rule for S128SwitchCaseFallthrough {
    ...
    fn title(&self) -> &str {
        "Switch cases should end with break"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
        ...
        for i in 0..lines.len().saturating_sub(1) {
            ...
            if (line.contains("case ") || line.ends_with(':')) && next_line.starts_with("case ") {
                ...
                if !line.contains("break")
                    ...
                    && !line.ends_with(':')
                {
                    // Allow empty fall-through for grouped cases
                    issues.push(create_issue(
                        self,
                        ctx.file_path,
                        i + 1,
...
impl Rule for S131SwitchDefault {
    ...
    fn title(&self) -> &str {
        "Switch should have a default case"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S134DeepNesting {
    ...
    fn title(&self) -> &str {
        "Control flow should not be nested too deeply"
    }
    fn severity(&self) -> Severity {
        Severity::Critical
    }
...
impl Rule for S135MultipleBreakContinue {
    ...
    fn title(&self) -> &str {
        "Loops should have single exit point"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S1066CollapsibleIf {
    ...
    fn title(&self) -> &str {
        "Collapsible ifs should be merged"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S1104PublicField {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"public\s+(\w+)\s+(\w+)\s*[;=]").unwrap());
        let excluded = [
            "class",
            "interface",
            "enum",
...
impl Rule for S1132StringLiteralOnLeft {
    ...
    fn title(&self) -> &str {
        "String literals should be on left side of equals()"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r#"(\w+)\.equals\s*\(\s*"[^"]*"\s*\)"#).unwrap());
        ...
        for (line_num, line) in ctx.source.lines().enumerate() {
            if RE.is_match(line) {
                issues.push(create_issue(
                    ...
                    line_num + 1,
                    1,
                    "Put the string literal on the left side to avoid NullPointerException."
                        .to_string(),
                    Some(line.trim().to_string()),
                ));
            }
...
impl Rule for S1135TodoComment {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)//\s*TODO|/\*\s*TODO").unwrap());
...
impl Rule for S1142TooManyReturns {
    ...
    fn title(&self) -> &str {
        "Methods should not have too many returns"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S1144UnusedPrivateField {
    ...
    fn title(&self) -> &str {
        "Unused private members should be removed"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
...
impl Rule for S1155UseCollectionIsEmpty {
    ...
    fn title(&self) -> &str {
        "Collection.isEmpty() should be used"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"\.size\(\)\s*(?:==|!=|>|<=)\s*0").unwrap());
...
impl Rule for S1181CatchThrowable {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static RE: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"catch\s*\(\s*(Throwable|Error|java\.lang\.Throwable|java\.lang\.Error)\s+")
                .unwrap()
        });
            ...
            for cap in RE.captures_iter(line) {
                if let Some(exc_type) = cap.get(1) {
                    issues.push(create_issue(
                        ...
                        exc_type.start() + 1,
                        format!(
                            "Catch a specific exception instead of '{}'.",
                            exc_type.as_str()
                        ),
                        Some(line.trim().to_string()),
                    ));
        ...
        for cap in RE.captures_iter(ctx.source) {
            if let Some(method) = cap.get(1) {
                ...
                issues.push(create_issue(
                    ...
                    1,
                    format!(
                        "Add implementation to empty method '{}' or add a comment.",
                        method.as_str()
                    ),
                    None,
                ));
        ...
        for (string, (line_num, count)) in string_counts {
            if count >= 3 {
                issues.push(create_issue(
                    ...
                    1,
                    format!(
                        "Define a constant for \"{}\" - used {} times.",
                        string, count
                    ),
                    None,
                ));
...
impl Rule for S1210EqualsHashCode {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        let has_equals = ctx.source.contains("boolean equals(Object")
            || ctx.source.contains("boolean equals(java.lang.Object");
...
smell_rule!(
    S800NestedComments,
    "S800",
    "Nested comments should not be used",
    Severity::Minor,
    r"/\*.*\*/.*\*/",
...
smell_rule!(
    S815UnicodeBom,
    "S815",
    "Source files should not contain BOM",
    Severity::Minor,
    r"^\xEF\xBB\xBF",
...
smell_rule!(
    S818LiteralSuffix,
    "S818",
    "Long literal suffixes should be uppercase",
    Severity::Minor,
    r"\d+l\b",
...
smell_rule!(
    S864OperatorPrecedence,
    "S864",
    "Operator precedence should be explicit",
    Severity::Major,
    r"[^&|]\s*[&|]\s*[^&|]",
...
smell_rule!(
    S881IncrementDecrement,
    "S881",
    "Increment/decrement operators should not be mixed",
    Severity::Major,
    r"\+\+.*\+\+|--.*--",
...
smell_rule!(
    S888LoopTermination,
    "S888",
    "Loop termination should use relational operators",
    Severity::Critical,
    r"for\s*\([^;]*;\s*\w+\s*!=\s*\w+\s*;",
...
smell_rule!(
    S923VoidReturnType,
    "S923",
    "Methods should have explicit return type",
    Severity::Minor,
    r"public\s+\w+\s*\([^)]*\)\s*\{",
            ...
            if line.contains("case ") || line.contains("default:") {
                if in_case && i - case_start > 10 {
                    issues.push(create_issue(
                        ...
                        case_start + 1,
                        1,
                        "This case block is too long. Extract to a method.".to_string(),
                        None,
                    ));
                }
                case_start = i;
...
smell_rule!(
    S1169EmptyCollectionReturn,
    "S1169",
    "Return empty collection instead of null",
    Severity::Major,
    r"return\s+null\s*;.*(?:Collection|List|Set|Map)",
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        if import_count > 30 {
            issues.push(create_issue(
                ...
                1,
                format!(
                    "This class depends on {} other classes. Reduce coupling.",
                    import_count
                ),
                None,
            ));
...
smell_rule!(
    S1213ParameterOrder,
    "S1213",
    "Members should be declared in order",
    Severity::Minor,
    r"private\s+\w+[^}]*public\s+\w+",
...
smell_rule!(
    S1258ParsingDouble,
    "S1258",
    "Use Double.parseDouble instead of valueOf",
    Severity::Minor,
    r"Double\.valueOf\s*\(",
...
impl Rule for S1479TooManyCases {
    ...
    fn title(&self) -> &str {
        "Switch statements should not have too many cases"
    }
    fn severity(&self) -> Severity {
        Severity::Major
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        if case_count > 30 {
            issues.push(create_issue(
                ...
                1,
                format!(
                    "This switch has {} cases. Consider using a Map or polymorphism.",
                    case_count
                ),
                None,
            ));
...
smell_rule!(
    S1640EntrySetIteration,
    "S1640",
    "Use entrySet() for iterating Map",
    Severity::Major,
    r"for\s*\([^:]+:\s*\w+\.keySet\(\)\)",
...
smell_rule!(
    S1710AnnotationNaming,
    "S1710",
    "Annotation types should have @interface",
    Severity::Major,
    r"public\s+class\s+\w+Annotation",
...
smell_rule!(
    S1820SubclassNaming,
    "S1820",
    "Subclasses should not duplicate parent names",
    Severity::Minor,
    r"class\s+\w+Base\s+extends\s+\w+Base",
...
smell_rule!(
    S2131StringValueOfPrimitive,
    "S2131",
    "Primitives in String valueOf",
    Severity::Minor,
    r"String\.valueOf\s*\(\s*\d+\s*\)",
    "Just write the literal as a string."
);

// S2142: InterruptedException handling
smell_rule!(
    S2143InterruptedHandling,
    "S2143",
    "InterruptedException should restore interrupt",
    Severity::Major,
...
smell_rule!(
    S2272IteratorRemove,
    "S2272",
    "Iterator.remove should be called correctly",
    Severity::Critical,
    r"Iterator[^{]*\{[^}]*\.remove\(",
...
smell_rule!(
    S2301SelectorArgument,
    "S2301",
    "Methods should not take boolean selector",
    Severity::Major,
    r"public\s+\w+\s+\w+\s*\([^)]*boolean\s+\w+Select",
    "Split this method into two - one for each boolean case."
);

// S2390: Subclasses accessing parent static
smell_rule!(
    S2390SubclassStaticAccess,
    "S2390",
    "Subclasses should not access parent static",
    Severity::Minor,
    r"extends\s+\w+[^{]+super\.\w+\s*=",
    "Access parent static members via parent class name."
);

// S2447: Null Boolean return
smell_rule!(
    S2447NullBooleanReturn,
    "S2447",
    "Boolean methods should not return null",
    Severity::Major,
    r"Boolean\s+\w+\s*\([^)]*\)\s*\{[^}]*return\s+null",
...
smell_rule!(
    S2583BTrueCondition,
    "S2583B",
    "Conditions should not always evaluate to true",
    Severity::Major,
    r"if\s*\(\s*1\s*==\s*1\s*\)",
...
smell_rule!(
    S2638ParameterChanged,
    "S2638",
    "Method overrides should not change parameter names",
    Severity::Minor,
    r"@Override[^{]+\(\s*\w+\s+\w+1\s*\)",
...
smell_rule!(
    S2692IndexOfCheck,
    "S2692",
    "indexOf result should be checked",
    Severity::Major,
    r"\.indexOf\s*\([^)]+\)\s*[><=!]=?\s*-?\d",
...
smell_rule!(
    S2698AssertionMessage,
    "S2698",
    "Assertions should include a message",
    Severity::Minor,
    r"assert\s+\w+\s*;",
...
smell_rule!(
    S2925ThreadSleepTest,
    "S2925",
    "Thread.sleep should not be used in tests",
    Severity::Major,
    r"@Test[^}]+Thread\.sleep\s*\(",
    "Use awaitility or similar instead of Thread.sleep in tests."
);

// S2959: Stream intermediate operations
smell_rule!(
    S2959StreamIntermediate,
    "S2959",
    "Stream intermediate operations should be collected",
    Severity::Major,
    r"\.stream\(\)\.(?:filter|map)\([^)]+\)\s*;",
...
smell_rule!(
    S3046RegexDos,
    "S3046",
    "Regex should not be vulnerable to DoS",
    Severity::Critical,
    r#"Pattern\.compile\s*\(\s*"[^"]*\(\.\*\)\*"#,
...
smell_rule!(
    S3060OverrideReturnType,
    "S3060",
    "Override should have same return type",
    Severity::Minor,
    r"@Override\s+public\s+Object\s+\w+\s*\(",
    "Consider using more specific return type in override."
);

// S3237: Value based equality
smell_rule!(
    S3237ValueBasedEquality,
    "S3237",
    "Value-based classes should not be compared by reference",
    Severity::Major,
    r"(?:LocalDate|LocalTime|LocalDateTime|Optional)\s+\w+\s*==",
    "Use equals() for value-based class comparison."
);

// S3254: Boolean expression
smell_rule!(
    S3254BooleanExpression,
    "S3254",
...
smell_rule!(
    S3257DeclareInitialize,
    "S3257",
    "Declarations should be initialized",
    Severity::Minor,
    r"(?:int|String|boolean|double|float|long)\s+\w+\s*;",
    "Consider initializing this variable at declaration."
);

// S3281: Superfluous type argument
smell_rule!(
    S3281SuperfluousTypeArg,
    "S3281",
    "Superfluous type argument",
    Severity::Minor,
    r"Collections\.<\w+>empty",
...
smell_rule!(
    S3330OptionalOrElse,
    "S3330",
    "Optional.orElse should not use method call",
    Severity::Major,
    r"\.orElse\s*\([^)]*\w+\(",
...
smell_rule!(
    ...
    Severity::Critical,
    r"synchronized\s*\(\s*getClass\(\)\s*\)",
    "Use ClassName.class for synchronization lock."
);

// S3655B: Optional unsafe access
smell_rule!(
    S3655OptionalUnsafe2,
    "S3655B",
    "Optional.get should be called only when present",
    Severity::Critical,
    r"Optional\w*\s*[^;]*\.get\(\)\s*;",
...
smell_rule!(
    S3878ArraysVarargs,
    "S3878",
    "Arrays should not be passed as varargs arguments",
    Severity::Minor,
...
smell_rule!(
    S3981CollectionSize,
    "S3981",
    "Collection size should not be compared to negative",
    Severity::Major,
    r"\.size\(\)\s*[<>=]+\s*-\d",
...
smell_rule!(
    S3986DateFormatConstants,
    "S3986",
    "Date format should be thread-safe",
    Severity::Major,
    r"static\s+(?:final\s+)?(?:Simple)?DateFormat",
    "DateFormat is not thread-safe, don't use as static."
);

// S4032: Packages at same depth
smell_rule!(
    S4032PackageSameDepth,
    "S4032",
    "Packages should be organized at same depth",
    Severity::Minor,
    r"package\s+\w+\.\w+\.\w+\.\w+\.\w+\.\w+",
    "Package nesting is too deep."
);

// S4065: Useless Thread.interrupted
smell_rule!(
    S4065UselessInterrupted,
    "S4065",
    "Thread.interrupted should be used properly",
    Severity::Major,
    r"Thread\.interrupted\(\)\s*;",
    "Thread.interrupted() return value should be checked."
);

// S4144: Duplicate methods (additional)
smell_rule!(
    S4144DuplicateMethod2,
    "S4144B",
    "Methods should not be duplicates",
    Severity::Major,
    r"void\s+\w+Copy\s*\([^)]*\)",
...
smell_rule!(
    S4165SelfAssign,
    "S4165B",
    "Assignment to self is useless",
    Severity::Major,
    r"this\.\w+\s*=\s*this\.\w+\s*;",
    "This assignment has no effect."
);

// S4276: Primitive types in generics
smell_rule!(
    S4276PrimitiveGenerics,
    "S4276B",
    "Use primitive stream types",
    Severity::Major,
    r"Stream<Integer>|Stream<Long>|Stream<Double>",
...
smell_rule!(
    S4288ArrayToString,
    "S4288",
    "Arrays should use Arrays.toString",
    Severity::Major,
    r"\[\]\.toString\(\)",
...
smell_rule!(
    S4351CompareToMinValue,
    "S4351B",
    "compareTo should not return MIN_VALUE",
    Severity::Major,
...
smell_rule!(
    S4423WeakTls,
    "S4423B",
    "Weak TLS protocols should not be used",
    Severity::Critical,
...
smell_rule!(
    S4544GenericWildcard,
    "S4544",
    "Unsafe use of generic wildcards",
    Severity::Major,
    r"<\?\s*super\s+\w+>",
...
smell_rule!(
    S4601UnicodeSeparator,
    "S4601",
    "Standard character sets should be used",
    Severity::Minor,
    r#"new\s+String\s*\([^)]+,\s*"UTF8"\s*\)"#,
    "Use StandardCharsets.UTF_8 instead."
);

// S4682: Primitive comparison
smell_rule!(
    S4682PrimitiveComparison,
    "S4682",
    "Primitives should not be compared with ==",
    Severity::Major,
    r"Integer\s+\w+\s*==\s*Integer|Long\s+\w+\s*==\s*Long",
    "Use equals() for wrapper comparison."
);

// S4784: Regex complexity
smell_rule!(
    S4784RegexComplexity,
    "S4784B",
    "Regex should not be too complex",
    Severity::Major,
    r#"Pattern\.compile\s*\(\s*"[^"]{80,}""#,
    "This regex is too complex, consider simplifying."
);
...
smell_rule!(
    S5042ZipEntry,
    "S5042",
    "Archive entry should be validated",
    Severity::Critical,
    r"ZipEntry[^}]+\.getName\(\)",
...
smell_rule!(
    S5128BeanValidation,
    "S5128",
    "Bean validation should be used",
    Severity::Minor,
    r"@(?:NotNull|NotEmpty|NotBlank)\s*\n\s*@",
...
smell_rule!(
    S5361ReplaceAll,
    "S5361B",
    "replaceAll uses regex, use replace for literal",
    Severity::Minor,
    r#"\.replaceAll\s*\(\s*"[^"]*(?:\\.|[^\\.])""#,
...
smell_rule!(
    S5663PublicTestField,
    "S5663B",
    "Test class fields should not be public",
    Severity::Minor,
    r"@Test[^}]+public\s+\w+\s+\w+\s*=",
...
smell_rule!(
    S6035UriBuild,
    "S6035",
    "URIs should be built with proper API",
    Severity::Major,
    r#"new\s+URI\s*\(\s*"[^"]*"\s*\+"#,
...
smell_rule!(
    S6126StringBuilderReplace,
    "S6126",
    "StringBuilder should use append",
    Severity::Minor,
    r"StringBuilder[^;]*\+=",
...
smell_rule!(
    S6201PatternFlagsConst,
    "S6201B",
    "Pattern flags should use constants",
    Severity::Minor,
    r"Pattern\.compile\s*\([^,]+,\s*\d+\s*\)",
...
smell_rule!(
    S6353EmptyAlt2,
    "S6353B",
    "Regex alternation should not be empty",
    Severity::Major,
    r#"Pattern\.compile\s*\(\s*"[^"]*\(\|\)""#,
...
smell_rule!(
    S1155BCollectionEmpty,
    "S1155B",
    "Use isEmpty() instead of size() == 0",
    Severity::Minor,
    r"\.length\s*==\s*0|\.length\s*>\s*0",
...
smell_rule!(
    ...
    "Unused method parameters should be removed",
    Severity::Major,
    r"public\s+\w+\s+\w+\s*\([^)]*\bunused\w+",
    "Remove or use this unused parameter."
);

// S1301B: Switch instead of if chain
smell_rule!(
    S1301BSwitchPreferred,
    "S1301B",
    "Switch should be used instead of if-else chain",
    Severity::Minor,
    r"if\s*\([^)]+==\s*\d+\)\s*\{[^}]*\}\s*else\s+if\s*\([^)]+==\s*\d+\)",
    "Use switch statement for multiple equality comparisons."
);

// S1302: Avoid using the same name for multiple constants
smell_rule!(
    S1302DuplicateConstName,
    "S1302",
    "Constants should have unique names",
    Severity::Minor,
    r"static\s+final\s+\w+\s+\w+\s*=[^;]*;[^}]*static\s+final\s+\w+\s+\w+\s*=",
    "Avoid reusing constant names across classes."
);

// S1303: Avoid string literals longer than N characters
smell_rule!(
    S1303LongStringLiteral,
    "S1303",
    "String literals should not be too long",
    Severity::Minor,
    r#""[^"]{100,}""#,
...
smell_rule!(
    S1315FixmeTicket,
    "S1315",
    "FIXME should have a ticket reference",
    Severity::Minor,
    r"(?i)FIXME\s*$|FIXME\s+[^A-Z]",
...
smell_rule!(
    S1316TodoTicket,
    "S1316",
    "TODO should have a ticket reference",
    Severity::Minor,
    r"(?i)TODO\s*$|TODO\s+[^A-Z]",
    "Add a ticket reference to this TODO comment."
);

// S1317: StringBuilder capacity
smell_rule!(
    S1317StringBuilderCap,
    "S1317",
    "StringBuilder should be created with capacity",
    Severity::Minor,
...
smell_rule!(
    S1319InterfaceType,
    "S1319",
    "Declarations should use interface types",
    Severity::Minor,
    r"(?:ArrayList|LinkedList|HashSet|TreeSet|HashMap|TreeMap)\s*<[^>]+>\s+\w+\s*=",
...
smell_rule!(
    S1320SimilarNames,
    "S1320",
    "Avoid similar class and interface names",
    Severity::Minor,
    r"interface\s+I\w+|class\s+\w+Impl\b",
...
smell_rule!(
    S1321SizeMax,
    "S1321",
    "Avoid comparison to SIZE_MAX",
    Severity::Major,
    r">\s*Integer\.MAX_VALUE|<\s*Integer\.MIN_VALUE",
...
smell_rule!(
    S1323ConstantCondition,
    "S1323",
    "Constant expressions in conditions",
    Severity::Major,
    r"if\s*\(\s*(?:true|false)\s*\)",
...
smell_rule!(
    S1324ArrayPosition,
    "S1324",
    "Array designator should be after type",
    Severity::Minor,
    r"\w+\s+\w+\[\]\s*(?:,|;|\))",
...
smell_rule!(
    S1325AnonymousRunnable,
    "S1325",
    "Use lambda instead of anonymous Runnable",
    Severity::Minor,
    r"new\s+Runnable\s*\(\s*\)\s*\{",
...
smell_rule!(
    S1329UseAddAll,
    "S1329",
    "Use addAll instead of loop with add",
    Severity::Minor,
    r"for\s*\([^)]+\)\s*\{[^}]*\.add\s*\(",
...
smell_rule!(
    S1330EmptyWhile,
    "S1330",
    "Empty while loops should be documented",
    Severity::Major,
...
smell_rule!(
    S1332NestedLoops,
    "S1332",
    "Deeply nested loops should be refactored",
    Severity::Major,
    r"for\s*\([^{]+\{[^}]*for\s*\([^{]+\{[^}]*for\s*\(",
...
smell_rule!(
    ...
    Severity::Minor,
    r"\.indexOf\s*\([^)]+\)\s*>=\s*0|\.indexOf\s*\([^)]+\)\s*!=\s*-1",
    "Use contains() instead of indexOf() >= 0."
);

// S1334: Format specifiers consistency
smell_rule!(
    S1334FormatSpecifiers,
    "S1334",
    "Format specifiers should be consistent",
    Severity::Minor,
    r#"String\.format\s*\(\s*"[^"]*%[sd][^"]*"#,
...
smell_rule!(
    ...
    Severity::Major,
    r"public\s+\w+\s+\w+\s*\([^)]*\w+\s+\w+[^)]*\)\s*\{[^}]*\w+\s*=\s*[^=]",
    "Avoid reassigning method parameters."
);

// S1336: Simplify boolean returns
smell_rule!(
    S1336BooleanSimplify,
    "S1336",
    "Boolean return can be simplified",
    Severity::Minor,
    r"if\s*\([^)]+\)\s*\{\s*return\s+true\s*;\s*\}\s*return\s+false",
...
smell_rule!(
    ...
    Severity::Major,
    r"catch\s*\(\s*(?:Exception|RuntimeException)\s+\w+\s*\)\s*\{",
    "Catch more specific exceptions."
);

// S1338: Use correct equals comparison
smell_rule!(
    S1338EqualsComparison,
    "S1338",
    "Potential equals type mismatch",
    Severity::Major,
    r"\.equals\s*\(\s*\d+\s*\)",
...
smell_rule!(
    S1340InsecureRandom,
    "S1340",
    "Random should not be used for security",
    Severity::Critical,
    r"new\s+Random\s*\(\s*\).*(?:password|token|secret|key)",
...
smell_rule!(
    ...
    Severity::Major,
    r"for\s*\([^)]+\)\s*\{[^}]*Integer\.valueOf|for\s*\([^)]+\)\s*\{[^}]*Long\.valueOf",
...
smell_rule!(
    S1343StringSplit,
    "S1343",
    "Use indexOf for one character split",
    Severity::Minor,
    r"\.split\s*\(\s*\S{3}\s*\)",
...
smell_rule!(
    S1344FloatEquals,
    "S1344",
    "Float values should not use equals",
    Severity::Major,
    r"(?:Float|Double)\s+\w+\s*==\s*\w+|\.equals\s*\([^)]*(?:Float|Double)",
...
smell_rule!(
    S1345NullInstanceof,
    "S1345",
    "Null check before instanceof is redundant",
    Severity::Minor,
    r"!=\s*null\s*&&\s*\w+\s+instanceof|instanceof[^&]+&&\s*\w+\s*!=\s*null",
    "instanceof already checks for null."
);

// S1346: Multiple string replacements
smell_rule!(
    S1346MultipleReplace,
    "S1346",
    "Multiple replacements should be combined",
    Severity::Minor,
...
smell_rule!(
    S1347PrimitiveStream,
    "S1347",
    "Use primitive stream operations",
    Severity::Minor,
    r"\.mapToInt\([^)]*Integer::intValue\)|\.mapToLong\([^)]*Long::longValue\)",
...
smell_rule!(
    S1348EmptyOptional,
    "S1348",
    "Consider returning Optional.empty()",
    Severity::Minor,
    r"return\s+Optional\.ofNullable\s*\(\s*null\s*\)",
...
smell_rule!(
    S1349MultipleReturn,
    "S1349",
    "Consider reducing return statements",
    Severity::Minor,
    r"return[^}]+return[^}]+return[^}]+return[^}]+return",
...
smell_rule!(
    S1351EmptyStringInit,
    "S1351",
    "Avoid empty string initialization",
    Severity::Minor,
    r#"String\s+\w+\s*=\s*"";"#,
...
smell_rule!(
    ...
    Severity::Major,
    r"\bList\s+\w+\s*=|\bMap\s+\w+\s*=|\bSet\s+\w+\s*=",
...
smell_rule!(
    S1353ConstantLeft,
    "S1353",
    "Constants on left side of comparison",
    Severity::Minor,
...
smell_rule!(
    S1354DeadStore,
    "S1354",
    "Variable value is overwritten before use",
    Severity::Major,
    r"\w+\s*=[^;]+;\s*\w+\s*=[^;]+;",
...
smell_rule!(
    S1356FieldAccessEquals,
    "S1356",
    "Use getters in equals implementation",
    Severity::Minor,
    r"equals\([^)]*\)\s*\{[^}]*other\.\w+\s*==",
    "Consider using getters instead of direct field access."
);

// S1357: Method chain too long
smell_rule!(
    S1357LongMethodChain,
    "S1357",
    "Method chain should not be too long",
    Severity::Minor,
    r"\.\w+\([^)]*\)\.\w+\([^)]*\)\.\w+\([^)]*\)\.\w+\([^)]*\)\.\w+\(",
...
smell_rule!(
    S1358UnusedException,
    "S1358",
    "Exception parameter should be used",
    Severity::Minor,
...
smell_rule!(
    S1359SpecificAnnotation,
    "S1359",
    "Annotation usage should be checked",
    Severity::Minor,
    r#"@SuppressWarnings\s*\(\s*"all"\s*\)"#,
    "Use specific warning names instead of all."
);

// S1360: Synchronized method or block
smell_rule!(
    S1360SynchronizedMethod,
    "S1360",
    "Consider using java.util.concurrent",
    Severity::Minor,
    r"public\s+synchronized\s+|synchronized\s*\(\s*this\s*\)",
    "Consider using java.util.concurrent classes."
);

// S1361: Interface with only constants
smell_rule!(
    S1361ConstantInterface,
    "S1361",
    "Constant interfaces should be avoided",
    Severity::Major,
    r"interface\s+\w+\s*\{[^}]*static\s+final[^}]*\}",
...
smell_rule!(
    S1363AbstractStatic,
    "S1363",
    "Abstract class has only static methods",
    Severity::Major,
    r"abstract\s+class\s+\w+\s*\{[^}]*(?:public|private|protected)\s+static[^}]*\}",
...
smell_rule!(
    S1364TestNaming,
    "S1364",
    "Test method should describe behavior",
    Severity::Minor,
    r"@Test\s+(?:public\s+)?void\s+test\d+\s*\(",
...
smell_rule!(
    S1365HardcodedPath,
    "S1365",
    "File paths should not be hardcoded",
    Severity::Major,
...
smell_rule!(
    S1367FieldHiding,
    "S1367",
    "Field hides parent class field",
    Severity::Major,
    r"extends\s+\w+[^{]+\{[^}]*private\s+\w+\s+name\s*;",
...
smell_rule!(
    ...
    Severity::Major,
    r"static\s+\w+\s+\w+\s*=[^;]+\w+\s*;[^}]*static\s+\w+\s+\w+\s*=",
...
smell_rule!(
    S1371IgnoredException,
    "S1371",
    "Exception should not be silently ignored",
    Severity::Major,
    r"catch\s*\([^)]+\)\s*\{\s*\}",
...
smell_rule!(
    ...
    Severity::Major,
    r"public\s+(?:int|String|boolean|Object|List|Map|Set)\s+\w+\s*[=;]",
    "Make fields private and use getters/setters."
);

// S1373: Commented out import
smell_rule!(
    S1373CommentedImport,
    "S1373",
...
smell_rule!(
    ...
    Severity::Minor,
    r"new\s+Date\s*\(\s*\)|import\s+java\.util\.Date",
...
smell_rule!(
    ...
    Severity::Minor,
    r#"(?:log|logger)\.\w+\s*\(\s*"\s*\+|String\.format\s*\([^)]+\)\s*\)"#,
    "Use parameterized logging instead of string concatenation."
);

// S1378: Use assertThat for collections
smell_rule!(
    S1378AssertCollection,
    "S1378",
    "Use assertThat for collection assertions",
    Severity::Minor,
    r"assertEquals\s*\(\s*\d+\s*,\s*\w+\.size\(\)",
...
smell_rule!(
    ...
    Severity::Major,
    r"public\s+class\s+\w+Util(?:s|ity)?\s*\{",
...
smell_rule!(
    S1380CommentDensity,
    "S1380",
    "Code should have adequate comments",
    Severity::Info,
    r"(?:public|private|protected)\s+\w+\s+\w+\s*\([^)]{50,}\)\s*\{",
...
smell_rule!(
    S1381InterfaceSize,
    "S1381",
    "Interface should not have too many methods",
    Severity::Major,
    r"interface\s+\w+\s*\{[^}]*void\s+\w+[^}]*void\s+\w+[^}]*void\s+\w+[^}]*void\s+\w+[^}]*void\s+\w+[^}]*void\s+\w+",
    "Split large interface into smaller, cohesive interfaces."
);

// S1382: Single responsibility
smell_rule!(
    S1382SingleResponsibility,
    "S1382",
    "Class may have too many responsibilities",
    Severity::Major,
    r"class\s+\w+[^{]+\{[^}]{5000,}",
...
smell_rule!(
    S1385ArrayFormat,
    "S1385",
    "Array initializer should be on one line",
    Severity::Minor,
    r"new\s+\w+\[\]\s*\{\s*\n",
...
smell_rule!(
    S1387AnnotationString,
    "S1387",
    "Duplicate string in annotation",
    Severity::Minor,
    r#"@\w+\s*\(\s*"[^"]+"\s*\)[^{]+@\w+\s*\(\s*"[^"]+"\s*\)"#,
...
smell_rule!(
    S1388VolatileArray,
    "S1388",
    "Volatile does not make array elements volatile",
    Severity::Critical,
    r"volatile\s+\w+\[\]\s+\w+",
...
smell_rule!(
    S1389EmptyMethod,
    "S1389",
    "Empty method should have comment",
    Severity::Minor,
    r"(?:public|private|protected)\s+void\s+\w+\s*\([^)]*\)\s*\{\s*\}",
    "Add comment explaining why method is empty."
);

// S1390: Prefer primitive streams
smell_rule!(
    S1390PrimitiveStream,
    "S1390",
...
smell_rule!(
    S1391ExcessiveImports,
    "S1391",
    "Too many imports may indicate coupling",
    Severity::Major,
    r"(?:import\s+[^;]+;\s*){50,}",
...
smell_rule!(
    S1392StringJoin,
    "S1392",
    "Use String.join for joining",
    Severity::Minor,
    r"StringBuilder[^}]+for[^}]+\.append\([^)]+\)\.append\s*\([^)]*delimiter",
...
smell_rule!(
    ...
    Severity::Critical,
    r"private\s+static\s+\w+\s+instance\s*;[^}]*public\s+static\s+\w+\s+getInstance",
...
smell_rule!(
    ...
    Severity::Major,
    r"try\s*\{[^}]*\w+1\s*=[^}]*\w+2\s*=[^}]*finally[^}]*\w+1\.close[^}]*\w+2\.close",
    "Close resources in reverse order of their creation."
);

// S1397: Use StandardCharsets
smell_rule!(
    S1397StandardCharsets,
    "S1397",
    "Use StandardCharsets constants",
    Severity::Minor,
    r#"getBytes\s*\(\s*"UTF-8"\s*\)|"UTF-8""#,
...
smell_rule!(
    ...
    Severity::Critical,
    r"synchronized[^}]+if\s*\(\s*\w+\s*==\s*null\s*\)[^}]*\w+\s*=\s*new",
...
smell_rule!(
    ...
    Severity::Minor,
    r"catch\s*\([^)]+\w+\s*\)\s*\{\s*throw\s+\w+\s*;\s*\}",
...
smell_rule!(
    S1400VariableShadow,
    "S1400",
    "Variable shadows outer variable",
    Severity::Major,
    r"for\s*\(\s*(?:int|String)\s+i[^{]+\{[^}]*(?:int|String)\s+i\s*[=;]",
...
smell_rule!(
    S1500ComplexSwitch,
    "S1500",
    "Switch expression is too complex",
    Severity::Major,
    r"switch\s*\([^)]+\)\s*\{[^}]{500,}",
...
smell_rule!(
    ...
    "Method is too long",
    Severity::Major,
    r"(?:public|private|protected)\s+\w+\s+\w+\s*\([^)]*\)\s*\{[^}]{1000,}",
...
smell_rule!(
    S1503DuplicateCall,
    "S1503",
    "Duplicate method call in expression",
    Severity::Minor,
    r"\.\w+\s*\([^)]*\)[^;]*\.\w+\s*\([^)]*\)[^;]*\.\w+\s*\(",
...
smell_rule!(
    S1504HardcodedDb,
    "S1504",
    "Database name should not be hardcoded",
    Severity::Major,
    r#"(?i)(?:jdbc|connection).*(?:mysql|postgres|oracle|sqlserver)"#,
    "Extract database configuration."
);

// S1505: Avoid hardcoded server names
smell_rule!(
    S1505HardcodedServer,
    "S1505",
    "Server name should not be hardcoded",
    Severity::Major,
    r#"(?i)"(?:localhost|127\.0\.0\.1|::1)(?::\d+)?""#,
    "Extract server configuration."
);

// S1506: Avoid magic numbers in comparisons
smell_rule!(
    S1506MagicComparison,
    "S1506",
    "Magic number in comparison",
    Severity::Minor,
    r"(?:==|!=|<|>|<=|>=)\s*\d{2,}",
...
smell_rule!(
    S1507EmptyConcat,
    "S1507",
    "Empty string concatenation is redundant",
    Severity::Minor,
    r#"\+\s*""|\"\"\s*\+"#,
...
smell_rule!(
    S1508InstanceofCast,
    "S1508",
    "Cast after instanceof is redundant",
    Severity::Minor,
    r"instanceof\s+\w+\s*\)[^;]*\(\s*\w+\s*\)",
...
smell_rule!(
    ...
    Severity::Minor,
    r"\.trim\(\)\.trim\(\)|\.toLowerCase\(\)\.toLowerCase\(\)",
...
smell_rule!(
    S1511SBCapacity,
    "S1511",
    "StringBuilder should have initial capacity",
    Severity::Minor,
...
smell_rule!(
    S1512UseFormat,
    "S1512",
    "Use String.format for complex concatenation",
    Severity::Minor,
...
smell_rule!(
    S1513MutableDefault,
    "S1513",
    "Avoid mutable default collections",
    Severity::Major,
    r"static\s+final\s+(?:List|Set|Map)\s*<[^>]+>\s+\w+\s*=\s*new",
...
smell_rule!(
    S1514SameTypeCast,
    "S1514",
    "Cast to same type is redundant",
    Severity::Minor,
    r"\(\s*String\s*\)\s*\w+\.toString\s*\(",
    "Remove redundant type cast."
);

// S1515: Avoid empty catch with continue
smell_rule!(
    S1515EmptyCatchContinue,
    "S1515",
    "Empty catch with continue",
    Severity::Major,
    r"catch\s*\([^)]+\)\s*\{\s*continue\s*;\s*\}",
    "Add logging before continue in catch."
);

// S1516: Avoid returning in finally
smell_rule!(
    S1516ReturnInFinally,
    "S1516",
    "Return in finally is dangerous",
    Severity::Blocker,
...
smell_rule!(
    ...
    Severity::Minor,
    r"(?:ArrayList|LinkedList|HashSet|HashMap|TreeMap)\s*<[^>]+>\s+\w+\s*=\s*new",
...
smell_rule!(
    ...
    Severity::Major,
    r"if\s*\(\s*\w+\s*\)[^}]*else\s+if\s*\(\s*\w+\s*\)",
    "Remove duplicate condition."
);

// S1520: Avoid unnecessary else
smell_rule!(
    S1520UnnecessaryElse,
    "S1520",
    "Else after return is unnecessary",
    Severity::Minor,
    r"return\s+[^;]+;\s*\}\s*else\s*\{",
...
smell_rule!(
    ...
    Severity::Major,
    r"\w+\s+\w+\s*,\s*\w+\s+\w+\s*,\s*\w+\s+\w+\s*,\s*\w+\s+\w+\s*,\s*\w+\s+\w+\s*\)",
...
smell_rule!(
    S1527FeatureEnvy,
    "S1527",
    "Method uses too many external methods",
    Severity::Major,
    r"\.\w+\s*\([^)]*\)[^}]*\.\w+\s*\([^)]*\)[^}]*\.\w+\s*\([^)]*\)[^}]*\.\w+\s*\([^)]*\)[^}]*\.\w+\s*\(",
...
smell_rule!(
    ...
    Severity::Minor,
    r"==\s*null\s*\?\s*\w+\s*==\s*null\s*:\s*\w+\.equals",
    "Use Objects.equals() for null-safe comparison."
);

// S1529: Avoid returning this in setter
smell_rule!(
    S1529SetterReturnsThis,
    "S1529",
    "Setter returns this for chaining",
    Severity::Info,
    r"void\s+set\w+\s*\([^)]+\)\s*\{[^}]*return\s+this",
    "Consider using builder pattern instead."
);

// S1530: Avoid public constructor in utility class
smell_rule!(
    S1530UtilConstructor,
    "S1530",
    "Utility class has public constructor",
    Severity::Major,
    r"class\s+\w+Utils?\s*\{[^}]*public\s+\w+Utils?\s*\(\s*\)",
    "Make utility class constructor private."
);

// S1531: Avoid static import of all members
smell_rule!(
    S1531StaticImportAll,
    "S1531",
    "Static import should not use wildcard",
    Severity::Minor,
...
smell_rule!(
    ...
    Severity::Major,
    r"catch[^}]*log[^}]*throw|catch[^}]*throw[^}]*log",
...
smell_rule!(
    ...
    Severity::Minor,
    r"for\s*\(\s*int\s+\w+\s*=\s*0\s*;[^)]+\.size\s*\(",
    "Use enhanced for loop for readability."
);

// S1540: Avoid reassigning loop variable
smell_rule!(
    S1540LoopVarReassign,
    "S1540",
    "Loop variable reassigned inside loop",
    Severity::Major,
    r"for\s*\([^)]+\w+\s*\+\+[^)]*\)\s*\{[^}]*\w+\s*=",
...
smell_rule!(
    S1546MeaningfulNames,
    "S1546",
    "Use meaningful variable names",
    Severity::Minor,
    r"(?:int|String|boolean)\s+[a-z]\s*[=;]",
...
smell_rule!(
    ...
    Severity::Minor,
    r"@Test[^}]+assert\w+[^}]+assert\w+[^}]+assert\w+[^}]+assert\w+[^}]+assert\w+",
...
smell_rule!(
    S1601TodoRef,
    "S1601",
    "TODO should reference issue",
    Severity::Minor,
    r"(?i)//\s*TODO\s*$|//\s*TODO\s+[^A-Z]",
...
smell_rule!(
    ...
    Severity::Major,
    r"(?:int|String)\s+(?:class|interface|enum)\s*[=;]",
    "Don't use reserved words as variable names."
);

// S1603: Avoid comparing strings with new String
smell_rule!(
    S1603NewStringCompare,
    "S1603",
    "Avoid new String in comparison",
    Severity::Major,
    r"\.equals\s*\(\s*new\s+String\s*\(",
...
smell_rule!(
    S1607NumericOverflow,
    "S1607",
    "Potential numeric overflow",
    Severity::Major,
    r"\*\s*\d{5,}|Integer\.MAX_VALUE\s*\+|Long\.MAX_VALUE\s*\+",
    "Check for numeric overflow."
);

// S1608: Avoid using volatile for mutable objects
smell_rule!(
    S1608VolatileMutable,
    "S1608",
    "Volatile on mutable object",
    Severity::Critical,
    r"volatile\s+(?:List|Map|Set|StringBuilder)\s*<",
...
smell_rule!(
    S1609ModifyParam,
    "S1609",
    "Method parameter should not be modified",
    Severity::Major,
    r"(?:public|private|protected)\s+\w+\s+\w+\s*\([^)]*final\s+\w+\s+\w+[^)]*\)\s*\{[^}]*\w+\s*=",
...
smell_rule!(
    S1610DefaultOnly,
    "S1610",
    "Interface has only default methods",
    Severity::Major,
    r"interface\s+\w+\s*\{\s*default\s+\w+\s+\w+\s*\([^)]*\)\s*\{[^}]*\}\s*\}",
...
smell_rule!(
    S1612BLambdaRef,
    "S1612B",
    "Lambda can be method reference",
    Severity::Minor,
    r"->\s*\w+\.\w+\s*\(\s*\)",
...
smell_rule!(
    S1613UnnecessaryToString,
    "S1613",
    "Unnecessary toString call",
    Severity::Minor,
    r"String\.valueOf\s*\(\s*\w+\.toString\s*\(",
...
smell_rule!(
    S1617RedundantNull,
    "S1617",
    "Redundant null assignment",
    Severity::Minor,
    r"\w+\s*=\s*null\s*;[^}]*\w+\s*=\s*[^n]",
...
smell_rule!(
    ...
    Severity::Major,
    r"Integer\s+\w+\s*==\s*Integer|Long\s+\w+\s*==\s*Long|Boolean\s+\w+\s*==\s*Boolean",
...
smell_rule!(
    S1626StaticRandom,
    "S1626",
    "Static Random may cause contention",
    Severity::Major,
    r"static\s+(?:final\s+)?Random\s+\w+\s*=",
...
smell_rule!(
    S1627PatternInLoop,
    "S1627",
    "Pattern creation in loop",
    Severity::Major,
    r"for\s*\([^{]+\{[^}]*Pattern\.compile\s*\(",
...
smell_rule!(
    ...
    Severity::Minor,
    r"for\s*\([^{]+\{[^}]*new\s+File\s*\(",
    "Consider batching file operations."
);

// S1629: Avoid Collections.synchronizedXxx
smell_rule!(
    S1629SynchronizedCollection,
    "S1629",
    "Use concurrent collections",
    Severity::Major,
    r"Collections\.synchronized(?:List|Set|Map)\s*\(",
...
smell_rule!(
    S1630WrapperInLoop,
    "S1630",
    "Wrapper creation in loop",
    Severity::Minor,
    r"for\s*\([^{]+\{[^}]*Integer\.valueOf\s*\(",
...
smell_rule!(
    S1631GetClassSync,
    "S1631",
    "Use class literal for sync",
    Severity::Critical,
    r"synchronized\s*\(\s*getClass\s*\(\s*\)\s*\)",
...
smell_rule!(
    S1636StringBuffer,
    "S1636",
    "Use StringBuilder in single thread",
    Severity::Minor,
    r"new\s+StringBuffer\s*\(",
    "Use StringBuilder for single-threaded code."
);

// S1637: Avoid Calendar.getInstance()
smell_rule!(
    S1637CalendarInstance,
    "S1637",
...
smell_rule!(
    ...
    Severity::Major,
    r"System\.gc\s*\(\s*\)|Runtime\.getRuntime\s*\(\s*\)\.gc\s*\(",
...
smell_rule!(
    S1644ExceptionMessage,
    "S1644",
    "Create exception message in catch",
    Severity::Minor,
    r"catch\s*\([^)]+\)\s*\{[^}]*new\s+\w+Exception\s*\([^)]*\+",
...
smell_rule!(
    S1646WaitNoSync,
    "S1646",
    "wait/notify requires synchronized",
    Severity::Blocker,
    r"[^{]*\.(?:wait|notify|notifyAll)\s*\(",
...
smell_rule!(
    S1649OptionalNull,
    "S1649",
    "Optional method returns null",
    Severity::Critical,
    r"Optional\s*<[^>]+>\s+\w+\s*\([^)]*\)\s*\{[^}]*return\s+null",
    "Return Optional.empty() instead."
);

// S1650: Avoid modifying collection while iterating
smell_rule!(
    S1650ModifyWhileIterating,
    "S1650",
    "Modifying collection while iterating",
    Severity::Critical,
    r"for\s*\([^)]+:\s*\w+\)[^}]*\w+\.(?:add|remove)\s*\(",
...
smell_rule!(
    S1652FinallyLeak,
    "S1652",
    "Potential resource leak in finally",
    Severity::Major,
    r"finally\s*\{[^}]*=\s*null[^}]*\}",
...
smell_rule!(
    S1658TimeInLoop,
    "S1658",
    "Time retrieval in loop",
    Severity::Minor,
    r"for\s*\([^{]+\{[^}]*currentTimeMillis\s*\(",
...
smell_rule!(
    ...
    Severity::Minor,
    r"(?:int|String|long|double)\s+\w+\s*,\s*\w+\s*[;=]",
    "Declare one variable per line."
);
...
smell_rule!(
    S1661UnnecessaryBox,
    "S1661",
    "Unnecessary boxing operation",
    Severity::Minor,
    r"Integer\.valueOf\s*\(\s*\d+\s*\)\.intValue\s*\(",
...
smell_rule!(
    S1662CatchError,
    "S1662",
    "Catching Error is dangerous",
    Severity::Critical,
    r"catch\s*\(\s*(?:Error|OutOfMemoryError|StackOverflowError)\b",
    "Don't catch Error types."
);

// S1663: Avoid reflection security bypass
smell_rule!(
    S1663ReflectionBypass,
    "S1663",
    "Reflection bypassing security",
    Severity::Critical,
    r"setAccessible\s*\(\s*true\s*\)",
...
smell_rule!(
    S1664AssertProduction,
    "S1664",
    "Assertion may be disabled",
    Severity::Major,
    r"assert\s+\w+\s*:",
...
smell_rule!(
    ...
    Severity::Major,
    r"clone\s*\(\s*\)\s*\{\s*return\s+new\s+",
...
smell_rule!(
    S1673HardcodedLocale,
    "S1673",
    "Hardcoded locale",
    Severity::Minor,
    r#"Locale\s*\.\s*(?:ENGLISH|US|UK|FRENCH)"#,
    "Use configurable locale."
);

// S1674: Avoid hardcoded timezone
smell_rule!(
    S1674HardcodedTimezone,
    "S1674",
    "Hardcoded timezone",
    Severity::Minor,
    r#"TimeZone\.getTimeZone\s*\(\s*"[^"]+""#,
    "Use configurable timezone."
);

// S1675: Avoid excessive inheritance
smell_rule!(
    S1675ExcessiveInherit,
    "S1675",
...
smell_rule!(
    S1677InnerInInterface,
    "S1677",
    "Inner class in interface",
    Severity::Minor,
    r"interface\s+\w+\s*\{[^}]*class\s+\w+",
...
smell_rule!(
    S1681RawReturn,
    "S1681",
    "Method returns raw type",
    Severity::Major,
    r"public\s+(?:List|Map|Set|Collection)\s+\w+\s*\(",
...
smell_rule!(
    S1683PublicArray,
    "S1683",
    "Public array field is mutable",
    Severity::Major,
    r"public\s+(?:static\s+)?(?:final\s+)?\w+\[\]\s+\w+",
...
smell_rule!(
    S1684EnumOnlyValues,
    "S1684",
    "Enum without behavior",
    Severity::Info,
    r"enum\s+\w+\s*\{\s*\w+(?:\s*,\s*\w+)*\s*\}",
    "Consider adding behavior to enum."
);

// S1685: Avoid static method with instance data
smell_rule!(
    S1685StaticInstanceData,
    "S1685",
    "Static method accesses instance",
    Severity::Major,
    r"static\s+\w+\s+\w+\s*\([^)]*\)\s*\{[^}]*this\.",
...
smell_rule!(
    S1686ReturnMutable,
    "S1686",
    "Returning mutable internal",
    Severity::Major,
    r"return\s+this\.\w+;[^}]*private\s+(?:List|Map|Set|Date)",
    "Return defensive copy."
);
...
smell_rule!(
    S1687StoreMutable,
    "S1687",
    "Storing mutable parameter",
    Severity::Major,
    r"this\.\w+\s*=\s*\w+;[^}]*\([^)]*(?:List|Map|Set|Date)\s+\w+",
...
smell_rule!(
    ...
    Severity::Minor,
    r"catch\s*\([^)]+\)\s*\{[^}]*catch\s*\([^)]+\)\s*\{[^}]*catch\s*\([^)]+\)\s*\{[^}]*catch",
...
smell_rule!(
    ...
    Severity::Major,
    r"catch\s*\(\s*RuntimeException\s+\w+\s*\)",
    "Catch more specific exceptions."
);
...
smell_rule!(
    ...
    Severity::Critical,
    r"static\s+(?:List|Map|Set|StringBuilder)\s*<",
...
smell_rule!(
    ...
    Severity::Blocker,
    r#"(?i)(?:secretKey|encryptionKey|aesKey)\s*=\s*"[^"]+""#,
    "Use secure key management."
);

// S1692: Avoid insecure protocol
smell_rule!(
    S1692InsecureProtocol,
    "S1692",
    "Insecure protocol",
    Severity::Critical,
    r#"(?:http|ftp)://[a-zA-Z]"#,
...
smell_rule!(
    ...
    Severity::Blocker,
    r"(?:setHostnameVerifier|TrustAllCerts|ALLOW_ALL)",
    "Never disable SSL verification."
);

// S1694B: Avoid abstract class without abstract methods
smell_rule!(
    S1694BAbstractNoMethods,
    "S1694B",
...
smell_rule!(
    ...
    Severity::Major,
    r"catch\s*\(\s*NullPointerException\b",
...
smell_rule!(
    ...
    Severity::Minor,
    r"catch\s*\(\s*NumberFormatException\b",
    "Consider validation instead."
);

// S1698B: Avoid instanceof followed by cast
smell_rule!(
    S1698BInstanceofCast,
    "S1698B",
    "Instanceof followed by cast",
    Severity::Minor,
