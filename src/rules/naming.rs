...
impl Rule for S115ConstantNaming {
    ...
    fn title(&self) -> &str {
        "Constant names should comply with UPPER_SNAKE_CASE"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static CONST_DECL: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(static\s+final|final\s+static)\s+\w+\s+(\w+)").unwrap());
        let mut issues = Vec::new();
...
impl Rule for S119TypeParameterNaming {
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        ...
        static PATTERN: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"^[A-Z]$|^[A-Z][a-zA-Z0-9]*T$").unwrap());
        let mut issues = Vec::new();
            ...
            for cap in m.captures {
                ...
                if !PATTERN.is_match(name) {
                    issues.push(create_issue(
                        ...
                        cap.node.start_position().column + 1,
                        format!(
                            "Rename type parameter '{}' to single letter or end with 'T'",
                            name
                        ),
                        Some(name.to_string()),
                    ));
...
impl Rule for S1312LoggerNaming {
    ...
    fn title(&self) -> &str {
        "Loggers should be named LOG or LOGGER"
    }
    fn severity(&self) -> Severity {
        Severity::Minor
    }
    ...
    fn check(&self, ctx: &AnalysisContext) -> Vec<Issue> {
        static LOGGER_DECL: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(Logger|Log)\s+(\w+)\s*=").unwrap());
        let mut issues = Vec::new();
                ...
                if let Some(name) = caps.get(2) {
                    ...
                    if !matches!(name_str, "LOG" | "LOGGER" | "log" | "logger") {
                        issues.push(create_issue(
                            ...
                            name.start() + 1,
                            format!(
                                "Rename logger '{}' to 'LOG', 'LOGGER', 'log', or 'logger'",
                                name_str
                            ),
                            Some(name_str.to_string()),
                        ));
...
naming_rule!(
    S1700BFieldClassName,
    "S1700B",
    "Field should not share class name",
    Severity::Major,
    r"class\s+\w+[^{]+\{[^}]*(?:private|public|protected)\s+\w+\s+\w+\s*[;=]",
...
naming_rule!(
    S1701BooleanGetter,
    "S1701",
    "Boolean getter should start with is",
    Severity::Minor,
    r"(?:public|protected)\s+boolean\s+get\w+\s*\(\s*\)",
...
naming_rule!(
    S1705FactoryNaming,
    "S1705",
    "Factory methods should be named consistently",
    Severity::Minor,
    r"public\s+static\s+\w+\s+(?:make|build|produce)\w+\s*\(",
    "Consider using 'create', 'of', or 'from' for factory methods."
);

// S1706: Exception class naming
naming_rule!(
    S1706ExceptionNaming,
    "S1706",
    "Exception classes should end with Exception",
    Severity::Major,
    r"class\s+\w+[a-z]\s+extends\s+\w*Exception",
...
naming_rule!(
    S1708EnumConstantNaming,
    "S1708",
    "Enum constants should be uppercase",
    Severity::Minor,
    r"enum\s+\w+\s*\{[^}]*[a-z]\w*\s*[,}]",
...
naming_rule!(
    S1709BuilderNaming,
    "S1709",
    "Builder methods should follow pattern",
    Severity::Minor,
...
naming_rule!(
    S1711AnnotationElement,
    "S1711",
    "Annotation elements should be camelCase",
    Severity::Minor,
    r"@interface\s+\w+\s*\{[^}]*\w+_\w+\s*\(\s*\)",
    "Annotation elements should use camelCase."
);

// S1712: Test class naming
naming_rule!(
    S1712TestClassNaming,
    "S1712",
    "Test class should end with Test",
    Severity::Minor,
    r"class\s+\w+[a-z]\s*\{[^}]*@Test",
...
naming_rule!(
    S1713LambdaNaming,
    "S1713",
    "Lambda parameters should be meaningful",
    Severity::Minor,
    r"->\s*\{[^}]{50,}.*[a-z]\s*->",
...
naming_rule!(
    S1714GenericNaming,
    "S1714",
    "Generic types should follow convention",
    Severity::Minor,
    r"<\s*[a-z][a-z0-9]+\s*>",
...
naming_rule!(
    S1715AnnotationConstant,
    "S1715",
    "Annotation constants should be uppercase",
    Severity::Minor,
    r"@interface\s+\w+\s*\{[^}]*static\s+final\s+\w+\s+[a-z]\w*",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:public|private|protected)\s+\w+\s+\w{35,}\s*\(",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:int|String|boolean|double|long)\s+\w{30,}\s*[;=]",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:public|private|protected)\s+\w+\s+(?:\w*(?:impl|mgr|util|cfg|ctx|svc|repo)\w*)\s*[;=(]",
...
naming_rule!(
    S1721BooleanField,
    "S1721",
    "Boolean field should start with is/has/can",
    Severity::Minor,
    r"(?:private|protected)\s+boolean\s+[a-z]\w*\s*[;=]",
    "Boolean fields should start with is, has, can, etc."
);

// S1722: Collection naming
naming_rule!(
    S1722CollectionNaming,
    "S1722",
    "Collection variable should be plural",
    Severity::Minor,
    r"(?:List|Set|Collection)<[^>]+>\s+[a-z]+[a-rt-z]\s*[;=]",
...
naming_rule!(
    ...
    Severity::Minor,
    r"Optional<[^>]+>\s+(?:maybe|opt|possible)\w*\s*[;=]",
...
naming_rule!(
    S1725StreamNaming,
    "S1725",
    "Stream variable naming",
    Severity::Minor,
    r"Stream<[^>]+>\s+\w+Stream\s*[;=]",
...
naming_rule!(
    S1726FutureNaming,
    "S1726",
    "Future variable naming",
    Severity::Minor,
    r"(?:Future|CompletableFuture)<[^>]+>\s+\w+Future\s*[;=]",
...
naming_rule!(
    S1727ImplNaming,
    "S1727",
    "Avoid Impl suffix for implementations",
    Severity::Minor,
    r"class\s+\w+Impl\s+implements",
...
naming_rule!(
    ...
    Severity::Minor,
    r"abstract\s+class\s+[A-Z][a-z]\w*\s*[{<]",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:final\s+)?class\s+\w+(?:Helper|Helpers)\s*\{",
    "Prefer 'Utils' suffix over 'Helper'."
);

// S1730: Constant collection naming
naming_rule!(
    S1730ConstantCollection,
    "S1730",
    "Constant collection naming",
    Severity::Minor,
    r"static\s+final\s+(?:List|Set|Map)\s+[A-Z_]+\s*=",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:int|String|Object)\s+(?:temp|tmp|t)\s*[;=]",
    "Avoid 'temp', 'tmp', or 't' for variable names."
);

// S1734: Data class field naming
naming_rule!(
    S1734DataFieldNaming,
    "S1734",
    "Data class field naming",
    Severity::Minor,
    r"(?:private|protected)\s+\w+\s+m[A-Z]\w*\s*[;=]",
...
naming_rule!(
    ...
    Severity::Minor,
    r"@Configuration\s+class\s+\w+[a-z]\s*\{",
    "Configuration classes should end with 'Config'."
);

// S1739: Test fixture naming
naming_rule!(
    S1739TestFixtureNaming,
    "S1739",
    "Test fixture naming convention",
    Severity::Minor,
    r"@BeforeEach\s+(?:public\s+)?void\s+(?:init|prepare|before)\w*\s*\(",
...
naming_rule!(
    ...
    Severity::Minor,
    r"(?:CompletableFuture|Future)<[^>]+>\s+[a-z]\w*\s*\(",
    "Consider 'Async' suffix for async methods."
);

// S1741: Event handler naming
naming_rule!(
    S1741EventHandlerNaming,
    "S1741",
    "Event handler naming convention",
    Severity::Minor,
    r"void\s+(?:on|handle)[a-z]\w*\s*\(",
...
naming_rule!(
    ...
    Severity::Minor,
    r"\(\s*(?:Consumer|Function|Supplier|Runnable)\s*<[^>]*>\s+cb\s*\)",
    "Use descriptive names for callback parameters."
);

// S1743: Predicate naming
naming_rule!(
    S1743PredicateNaming,
    "S1743",
    "Predicate variable naming",
    Severity::Minor,
    r"Predicate<[^>]+>\s+[a-z]\w*\s*[;=]",
...
mod tests {
    ...
    fn test_s115_good_constant_names() {
        ...
        assert!(
            issues.is_empty(),
            "Good constant names should not be flagged"
        );
    }
